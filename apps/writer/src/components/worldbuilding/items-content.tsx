...
import { Loader2, Plus, Gem, Trash2, X } from "lucide-react";
import { toast } from "sonner";

// --- Badge helpers ---

const importanceBadgeColors: Record<string, string> = {
  MCGUFFIN: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-300",
...
function ItemsInner({ projectId }: { projectId: string }) {
  const { data, isLoading } = useItems(projectId);
  ...
  const [formImportance, setFormImportance] = useState("MODERATE");
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success("Oggetto creato con successo");
      setShowDialog(false);
      setFormName("");
      setFormType("");
      setFormImportance("MODERATE");
      setFormDescription("");
    } catch (err) {
      toast.error(
  ...
  async function handleDelete(id: string, name: string) {
    if (!confirm(`Eliminare l'oggetto "${name}"?`)) return;
    try {
    ...
    <div className="p-6">
      ...
      {items.length === 0 && (
        <div className="flex h-64 flex-col items-center justify-center gap-3 text-center">
          <Gem className="h-12 w-12 text-gray-300 dark:text-gray-600" />
          <div>
            <h3 className="font-medium">Nessun oggetto ancora</h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Aggiungi oggetti importanti, artefatti e strumenti del tuo mondo.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item: Item) => (
            ...
            >
              ...
              {/* Name */}
              <h3 className="pr-8 font-semibold">{item.name}</h3>

              {/* Info row */}
              <div className="mt-2 flex flex-wrap items-center gap-2">
                {item.type && (
                  <span className="inline-block rounded-full bg-violet-100 px-2 py-0.5 text-xs font-medium text-violet-800 dark:bg-violet-900/40 dark:text-violet-300">
                    {item.type}
                  </span>
                )}
                <span
                  className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${importanceBadgeColors[item.importance] ?? importanceBadgeColors.MODERATE}`}
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="mx-4 w-full max-w-md rounded-lg border bg-white p-6 shadow-xl dark:border-gray-700 dark:bg-gray-900">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold">Nuovo Oggetto</h2>
              <button
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800"
                  placeholder="es. Arma, Artefatto, Pozione..."
                />
              </div>
...
export function ItemsContent() {
  return (
    <ProjectSelector>
      {(projectId) => <ItemsInner projectId={projectId} />}
    </ProjectSelector>
  );
}
